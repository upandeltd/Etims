...
class KRAClient:
	...
	def post(self, endpoint, payload, reference_doctype=None, reference_name=None, require_auth=True):
		...
		max_retries = (
			self.settings.get("max_retry_attempts", 3) if self.settings.get("enable_retry_logic") else 1
		)
		...
		for attempt in range(max_retries):
			try:
				...
				if not response.content or not response.content.strip():
					...
					return {
						"Error": f"KRA API returned empty response (HTTP {response.status_code})",
						"Retryable": True,
					}
				result = self._handle_response(response.json(), endpoint)
				if "Success" in result:
					self._log_api_call(
						endpoint, reference_doctype, reference_name, "000", "Success", "success"
					)
				else:
					self._log_api_call(
						endpoint,
				...
				self._log_error(
					endpoint, f"Invalid JSON response (HTTP {response.status_code}): {response.text[:500]}"
				)
				return {
					"Error": f"KRA API returned invalid JSON (HTTP {response.status_code})",
					"Retryable": True,
				}
				...
				return {"Error": f"KRA API error: {e!s}", "Retryable": False}
			...
			frappe.get_doc(
				{
					...
					"output": frappe.as_json(
						{"resultCd": str(result_cd), "resultMsg": str(result_msg)[:500]}
					),
