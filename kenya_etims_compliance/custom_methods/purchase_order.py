...
def send_purchase_order_to_etims(purchase_order_name):
	...
	try:
		...
		existing = frappe.db.exists("eTIMS Purchase Order Tracking", {"purchase_order": purchase_order_name})

		if existing:
			return {"success": True, "tracking_id": existing, "message": "Purchase Order already tracked"}
		...
		return {"success": False, "error": str(e), "message": f"Failed to track Purchase Order: {e!s}"}
...
def reconcile_purchase_with_invoice(invoice_name):
	...
	try:
		...
		if not invoice.purchase_order:
			return {"reconciled": False, "message": "No purchase order linked to this invoice"}

		# Find tracking record
		tracking = frappe.db.get_value(
			"eTIMS Purchase Order Tracking", {"purchase_order": invoice.purchase_order}
		)
		...
		return {"reconciled": False, "message": "No tracking record found for this Purchase Order"}

	except (frappe.DoesNotExistError, frappe.ValidationError) as e:
		eTIMS.log_errors("PO Reconciliation Error", str(e))
		return {"reconciled": False, "error": str(e), "message": f"Failed to reconcile: {e!s}"}
...
def update_po_payment_status(invoice_name, payment_entry):
	...
	try:
		...
		if not invoice.purchase_order:
			return {"success": True, "message": "No PO linked - nothing to update"}
...
def get_po_tracking_summary(purchase_order):
	...
	try:
		...
		if not tracking:
			return {"tracked": False, "status": "Not Tracked"}
...
def get_all_unreconciled_pos():
	...
	try:
		tracking_records = frappe.db.get_all(
			...
			filters={"status": ["in", ["Pending Invoice", "Invoiced"]]},
			fields=[
				"name",
				"purchase_order",
				"supplier",
