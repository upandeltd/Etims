...
def run_stock_reconciliation(from_date, to_date, branch=None):
	...
	# Items in KRA
	for _key, kra in kra_summary.items():
	...
	kra_erpnext_codes = {
		etims_to_erpnext.get(kra["item_code"], kra["item_code"]) for kra in kra_summary.values()
	}
...
def _get_kra_summary(from_date, to_date, branch=None):
	...
	kra_entries = frappe.get_all(
		"eTIMS Stock Register Entry",
		filters=filters,
