...
def create_etims_item_data(doc, method):
    ...
    if doc.custom_update_item_to_tims == 1 and not doc.custom_registered_in_tims == 1:
        ...
        if exists:
            return {"message": f'eTIMS Item {exists} Exists!'}
...
def item_code_increment(doc_name):
    item_code_list = []
    item_codes_dgt_list = []
    tims_item_codes = frappe.db.get_all("eTIMS Item",
                                        filters = {'name': ['!=', doc_name]},
                                        fields=['etims_item_code']
                                    )
    if tims_item_codes:
        for item in tims_item_codes:
            if  item.get("etims_item_code"):
                if not item.get("etims_item_code") in item_code_list and len(item.get("etims_item_code")) > 10:
                    item_code_list.append(item.get("etims_item_code"))
...
def create_item_digit_code(doc_name):
    item_codes = item_code_increment(doc_name)
