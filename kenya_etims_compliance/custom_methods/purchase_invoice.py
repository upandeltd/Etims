...
def stockIOSaveReq(doc, date_str, item_count, reg_user_name, mod_user_name):
    ...
    if doc.custom_update_purchase_in_tims:
        try:
            ...
            try:
                for pur_item in doc.items:
                    eTIMS.stockMasterSaveReq(pur_item, doc.owner, reg_user_name, doc.modified_by, mod_user_name)
                    pur_item.custom_stock_master_updated = 1
