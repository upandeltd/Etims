...
def check_invoice_validity(invoice_no, supplier_pin, invoice_date, total_amount):
	...
	try:
		...
		return {"valid": False, "error": str(e), "message": f"An error occurred during verification: {e!s}"}
...
def get_verification_status(purchase_invoice):
	...
	try:
		...
		if invoice_verified:
			...
			return {"verified": False, "message": "Invoice has not been verified with KRA"}

	except frappe.DoesNotExistError as e:
		return {"verified": False, "error": str(e), "message": f"Error checking verification status: {e!s}"}
