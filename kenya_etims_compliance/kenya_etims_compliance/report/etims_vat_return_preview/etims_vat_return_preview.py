...
def get_data(filters):
	...
	sales_invoices = frappe.get_all("Sales Invoice", filters=si_filters,
		fields=["name", "base_grand_total", "base_total_taxes_and_charges",
	...
	output_taxable = sum(flt(s.base_grand_total) - flt(s.base_total_taxes_and_charges) for s in si_transmitted)
	...
	purchase_invoices = frappe.get_all("Purchase Invoice", filters=pi_filters,
		fields=["name", "base_grand_total", "base_total_taxes_and_charges",
				"custom_kra_match_status", "custom_update_purchase_in_tims"],
		limit_page_length=0)
	...
	supported_input_taxable = sum(flt(p.base_grand_total) - flt(p.base_total_taxes_and_charges) for p in pi_matched)
...
def get_summary(data):
	...
	return [
		...
		{"value": supported_row.get("tax_amount", 0), "label": _("Supported Input VAT"),
		 "datatype": "Currency", "indicator": "green"},
		{"value": at_risk_row.get("tax_amount", 0), "label": _("At-Risk Input VAT"),
		 "datatype": "Currency", "indicator": "red"},
		{"value": net_row.get("tax_amount", 0), "label": _("Net VAT Payable"),
