...
def get_columns():
	return [
		...
		{"fieldname": "document", "label": _("Document"), "fieldtype": "Dynamic Link",
		 "options": "document_type", "width": 180},
...
def get_data(filters):
	...
	for si in frappe.get_all("Sales Invoice", filters=si_filters, fields=[
		"name", "posting_date", "customer", "base_grand_total",
		"base_total_taxes_and_charges", "custom_invoice_number",
		"custom_update_invoice_in_tims", "custom_update_sales_to_etims",
	], order_by="posting_date desc", limit_page_length=5000):
		...
		data.append({
			"document_type": "Sales Invoice", "document": si.name,
			"posting_date": si.posting_date, "party": si.customer,
			"amount": si.base_grand_total, "tax_amount": si.base_total_taxes_and_charges,
			"etims_status": status, "invoice_number": si.custom_invoice_number,
			"backed": backed,
		})
	...
	for pi in frappe.get_all("Purchase Invoice", filters=pi_filters, fields=[
		"name", "posting_date", "supplier", "base_grand_total",
		"base_total_taxes_and_charges", "custom_invoice_number",
		...
		data.append({
			"document_type": "Purchase Invoice", "document": pi.name,
			"posting_date": pi.posting_date, "party": pi.supplier,
			"amount": pi.base_grand_total, "tax_amount": pi.base_total_taxes_and_charges,
			"etims_status": status, "invoice_number": pi.custom_invoice_number,
			"backed": backed,
		})
...
def get_summary(data):
	...
	backed_income = sum(d["amount"] for d in data
						if d["document_type"] == "Sales Invoice" and d["backed"] == "Yes")
	backed_expense = sum(d["amount"] for d in data
						 if d["document_type"] == "Purchase Invoice" and d["backed"] == "Yes")
	...
	return [
		...
		{"value": backed_income, "label": _("Backed Income"), "datatype": "Currency",
		 "indicator": "green"},
		{"value": total_expense, "label": _("Total Expenses"), "datatype": "Currency"},
		{"value": backed_expense, "label": _("Backed Expenses"), "datatype": "Currency",
		 "indicator": "green"},
		{"value": compliance, "label": _("Compliance %"), "datatype": "Percent",
