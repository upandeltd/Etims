					...
					if not item.get("stock_updated") == 1:
						try:
							...
							frappe.db.set_value(
								...
								self.name,
								{"stock_updated": 1, "stock_entry": new_stock_doc.name},
								update_modified=True,
							)

						except (frappe.ValidationError, frappe.DoesNotExistError) as e:
							frappe.log_error(title="eTIMS: Stock movement entry failed", message=str(e))
							frappe.throw(str(e))
			else:
				frappe.logger().debug("eTIMS: Stock movement has no items")
		else:
			frappe.throw(_("Warehouse not found for tax branch!"))
			...
			if self.customer_branch:
				if self.customer_branch in user_branch_list:
					if not self.customer_branch == "00":
						source_warehouse = frappe.db.get_all(
							"Warehouse",
							filters={
						...
						target_warehouse = frappe.db.get_all(
							...
							filters={
								"warehouse_type": "Stores",
								"is_group": 0,
								"custom_tax_branch_office": self.customer_branch,
							},
							fields=["warehouse_name", "name"],
						)
						...
						source_warehouse = frappe.db.get_all(
							...
							filters={
								"warehouse_type": "Stores",
								"is_group": 0,
								"custom_tax_branch_office": user_branch_list[0],
							},
							fields=["warehouse_name", "name"],
						)
...
def get_user_branch_perms():
	...
	if user_branch_permissions:
		for item in user_branch_permissions:
			if item.get("for_value") not in user_branch_list:
