...
class eTIMSCodeInformation(Document):
	...
	def codeSearchReq(self):
		...
		try:
			...
			frappe.log_error(title="eTIMS: Code Search failed", message=traceback.format_exc())
	...
	def custSearchReq(self):
		...
		payload = {"custmTin": self.customer_tin}
		...
		try:
			...
			frappe.log_error(title="eTIMS: Customer Search failed", message=traceback.format_exc())
			return {"Error": "Oops Bad Request!"}
	...
	def noticeSearchReq(self):
		...
		try:
			...
			frappe.log_error(title="eTIMS: Notice Search failed", message=traceback.format_exc())
			return {"Error": "Oops Bad Request!"}
	...
	def noticeInfoReq(self, notice_no):
		...
		for key, value in response.items():
			if key == "Success":
				...
				eTIMS.log_errors("Notice Info", value)
				return {"Error": value}
...
def check_if_notice_exists(notice_no):
	notice_exists = False
	notice_items = frappe.db.get_all("eTIMS Notice Item", filters={"notice_number": notice_no})
