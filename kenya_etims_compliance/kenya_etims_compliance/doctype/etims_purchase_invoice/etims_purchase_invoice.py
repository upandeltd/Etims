			...
			if not item.get("purchase_invoice_created") == 1:
				try:
					...
					frappe.db.set_value(
						...
						self.name,
						{
							"erpnext_purchase_invoice_updated": 1,
							"erpnext_purchase_invoice": new_purchase_doc.name,
						},
						update_modified=True,
					)
					...
					frappe.log_error(title="eTIMS: Purchase invoice processing error", message=str(e))
					frappe.throw(traceback.format_exc())

	def create_supplier(self):
		if not self.supplier_name:
			# Skip supplier creation if supplier_name is not provided
			return
...
def create_buying_price_list(item):
	...
	if not price_list_exists:
		...
		price_list = frappe.get_doc("Item Price", price_list_exists)
