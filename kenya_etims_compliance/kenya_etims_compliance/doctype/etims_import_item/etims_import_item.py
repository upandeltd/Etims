...
def create_and_link_erpnext_stock_entry(item):
	...
	if not item.get("stock_entry_created") == 1:
		stk_doc_exists = frappe.db.exists(
			"Stock Entry", {"custom_task_code": item.get("task_code"), "custom_is_import_stock": 1}
		)
		...
		if not stk_doc_exists:
			...
			try:
				...
				frappe.db.set_value(
					"eTIMS Import Item",
					item.name,
					{"stock_entry_created": 1, "stock_entry": new_stk_entry_doc.name},
					update_modified=True,
				)

			except (frappe.ValidationError, frappe.DoesNotExistError) as e:
				frappe.log_error(title="eTIMS: Import item stock entry failed", message=str(e))
...
def create_buying_price_list(item):
	item_price_ksh = (item.get("invoice_foreign_currency_amount") / item.get("package")) * item.get(
		"invoice_foreign_currency_crt"
	)

	price_list_exists = frappe.db.exists(
		"Item Price", {"item_code": item.item_name, "price_list": "Standard Buying"}
	)
...
def assign_stock_item(item):
	item_dict = {"item_code": item.get("item_name"), "qty": item.get("quantity")}
...
def get_target_warehouse():
	...
	target_warehouse = frappe.db.get_all(
		"Warehouse",
		filters={"warehouse_type": "Stores", "is_group": 0, "custom_tax_branch_office": user_branch},
...
def get_user_branch_perms():
	...
	user_branch_permissions = frappe.db.get_all(
		"User Permission",
		filters={"user": cur_user, "allow": "Tax Branch Office", "is_default": 1},
		fields=["for_value"],
	)
