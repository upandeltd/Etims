...
class eTIMSImportItemInformation(Document):
	...
	def importItemSearchReq(self):
		...
		try:
			...
			frappe.log_error(title="eTIMS: Search Import Item failed", message=traceback.format_exc())
...
def check_import_item_exits(task_code):
	...
	import_item = frappe.db.get_all(
		"eTIMS Import Item", filters={"task_code": task_code}, fields=["task_code"]
	)
...
def map_import_item(item):
	...
	try:
		item_price_ksh = (item.get("invoice_foreign_currency_amount") / item.get("quantity")) * item.get(
			"invoice_foreign_currency_crt"
		)
	except (ZeroDivisionError, TypeError, ValueError) as e:
		frappe.log_error(title="eTIMS: Import Item Price Calculation failed", message=str(e))
		item_price_ksh = 0
...
def get_etims_country(country_code):
	country_code_list = frappe.db.get_all(
		"eTIMS Country", filters={"code_name": country_code}, fields=["country_name"]
	)
...
def create_or_update_price_list(item_code, item_price):
	buying_price_list_exists = frappe.db.exists(
